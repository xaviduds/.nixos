...
read -p "[I]nstall | After-install and [S]SH | [C]lone repos and build: " -r answer
...
I)
    ...
    nixos-install --root /mnt --flake /mnt/etc/nixos#default && reboot ;;
S)
...
C)
    ...
    nix flake --extra-experimental-features "nix-command flakes" update ~/.nixos/ && export NIXPKGS_ALLOW_UNFREE=1 && nixos-rebuild boot --flake ~/.nixos#default --impure && reboot ;;
esac
